...
- [ ] T053 [P] Add `README.ru.md` — Russian translation of the finished root `README.md` (do this last, once T048's content is final, so it's translating the finished doc rather than something that'll drift), with a language-switch link at the top of both files — **explicitly deferred**: user requested this be dropped ("skip it then") in an earlier session; left unchecked (not done) rather than marked complete, in case it's revisited later
