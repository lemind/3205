...
- [x] T050 Run the full [quickstart.md](quickstart.md) manual walkthrough end-to-end; fix any discrepancies found — steps 1-4 (submit, watch progress with the 5-concurrency cap, browse, switch-without-stale) passed as specified, verified live in a real browser. Step 5 (cancel) surfaced a genuine bug: `job.status` flips to `cancelled` immediately on `DELETE`, well before the in-flight checks it let finish (and the still-queued ones waiting for a slot) actually converge — both `JobDetail` and `JobList` stopped polling on that immediate status flip, freezing the UI on stale `in_progress`/`pending` rows for as long as those checks took. Fixed by keying both widgets' poll-stop condition on per-URL/count completeness instead of `job.status`: added a `cancelledCount` field to `JobSummaryResponse` (backend model, service, `contracts/openapi.yaml`, `data-model.md`, frontend model) since the existing summary counts couldn't distinguish "still running" from "cancelled"; `JobList` now stops polling once `successCount + errorCount + cancelledCount === urlCount`, `JobDetail` once every result is terminal. Verified live: both views now keep updating through to final convergence instead of freezing. Also corrected a stale `data-model.md` validation note left over from T019's addendum.
- [x] T051 [P] Add a global exception filter in `backend/src/main.ts` and surface RTK Query `error`/`isLoading` states in the frontend widgets — `AllExceptionsFilter` (`backend/src/all-exceptions.filter.ts`) passes `HttpException`s through unchanged (validation/404 responses stay exactly as-is, verified live) and converts anything else into a generic 500 with server-side logging, so an unexpected bug can't leak an inconsistent body or internal details to the client; 2 unit tests. Audited all four interactive widgets for `error`/`isLoading`: `CreateJobForm`, `JobList`, `JobDetail` already handled both; `CancelJobButton` handled `isLoading` but silently swallowed a failed cancel — fixed to show the real backend message via the shared `getApiErrorMessage` helper (same one `CreateJobForm` uses).
- [x] T052 Final `docker-compose up --build` smoke test from a clean checkout (verifies SC-005) — both images built fresh (`docker compose up --build -d`), backend passed its healthcheck before frontend started (per the `depends_on: condition: service_healthy` wiring from ADR-0001). Verified through the real exposed ports: `:3000/api/health` direct, `:8080/` frontend served, `:8080/api/health` proxied through nginx, and a full create-job round trip through the proxy (`cancelledCount` field present, confirming T050's fix shipped in the image). Opened `:8080` in a real browser: renders correctly, zero console errors, job reached `completed` with correct counts. Stack torn down afterward (`docker compose down`) to restore the local dev setup already in use.
- [x] T053 [P] Add `README.ru.md` — Russian translation of the finished root `README.md` (do this last, once T048's content is final, so it's translating the finished doc rather than something that'll drift), with a language-switch link at the top of both files — full translation of every section (tech stack, coverage numbers, conventions, versions, setup/run, docs); `README.md` and `README.ru.md` cross-link at the top (`[Русский](README.ru.md)` / `[English](README.md)`). Previously deferred per an earlier "skip it then," now done per explicit instruction.
- [x] T054 [P] Add EN/RU UI language support to the frontend, RU default, with a switcher — [ADR-0008](../../docs/adr/0008-frontend-i18n.md): custom context + dictionary (`frontend/src/shared/i18n/`), no library, given the small (~30) string count. `LanguageSwitcher` (RU/EN toggle) in `JobsPage` next to the title; language persisted to `localStorage`, defaults to RU. All UI strings (labels, buttons, table headers, error fallbacks, the two count/progress templates) and status enum labels now go through `useTranslation()`. Verified live in both languages via screenshots, including the glitch-title CSS effect (which reads `data-text` — now derived from the translated title instead of hardcoded English).
