...
- URL validation on submission checks well-formedness (protocol + valid host/TLD after normalization), rejecting the whole request with 400 if any entry fails, so a typo can't waste a check slot only to resolve as a DNS/parse error later. A bare domain with no scheme (e.g. `google.com`) is normalized (`https://` prepended) before this check, since `fetch()` requires an absolute URL — this is the one common case worth smoothing over rather than rejecting or surfacing a confusing parse error. This is still a lightweight, syntax-only check (`validator`'s `isURL`) — it does not verify the host is reachable, only that it's a plausible URL.
