...
- URL validation on submission is minimal (non-empty lines); malformed URLs are allowed through and simply surface as `error` results from the `HEAD` check. One normalization is applied: a bare domain with no scheme (e.g. `google.com`) gets `https://` prepended, since `fetch()` requires an absolute URL — this is the one common case worth smoothing over rather than surfacing as a confusing parse error.
