...
- Validate all AI outputs through a schema (e.g. Zod).
- Prefer cheaper models first.
- Keep prompts centralized and versionable.
- Never add prompts, model IDs, or LLM API keys to a public repo; keep AI credentials and prompt logic in a private package/module, never inline them in shared code.
...
After **any** change that affects behavior, scope, architecture, stack, file layout, env vars, or delivery status, update the matching artifacts under `specs/` for the active feature (see `.specify/feature.json` → `feature_directory`, e.g. `specs/001-example-feature/`).
